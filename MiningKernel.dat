...
#pragma unroll
    ...
    // NUM_SIZE_8 bytes
    __local cl_uint *sk = (__local cl_uint *)sdata;
    ...
    for (int t = 0; t < NONCES_PER_THREAD; ++t)
    {
        *ctx = *((ctx_t __local *)(sdata + NUM_SIZE_32));
        ...
        if (tid < NONCES_PER_ITER)
        {
            ...
            CV = fn_Add(((cl_uint *)&base)[0], tid, 0, &non[0]);

            //asm volatile ("add.cc.u32 %0, %1, %2;":"=r"(non[0]): "r"(((cl_uint *)&base)[0]), "r"(tid));

            non[1] = 0;
            CV = fn_Add(((cl_uint *)&base)[1], 0, CV, &non[1]);
...
#pragma unroll
            ...
            {
                CV = fn_Add(r[0], hashes[ind[k] << 3], 0, &r[0]);
                //asm volatile ("add.cc.u32 %0, %0, %1;":"+r"(r[0]): "r"(hashes[ind[k] << 3]));

#pragma unroll
                for (int i = 1; i < 8; ++i)
                {
                    CV = fn_Add(r[i], hashes[(ind[k] << 3) + i], CV, &r[i]);
...
#pragma unroll
            ...
            //================================================================//
            CV = fn_mulLow(d[0], (cl_uint)q0_s, &med[0]);
            //asm volatile ("mul.lo.u32 %0, %1, " q0_s ";": "=r"(med[0]) : "r"(*d));
            CV = fn_mulHi(d[0], (cl_uint)q0_s, CV, &med[1]);
            //asm volatile ("mul.hi.u32 %0, %1, " q0_s ";": "=r"(med[1]) : "r"(*d));
            CV = fn_mulLow(d[0], (cl_uint)q2_s, &med[2]);
            //asm volatile ("mul.lo.u32 %0, %1, " q2_s ";": "=r"(med[2]) : "r"(*d));
            CV = fn_mulHi(d[0], (cl_uint)q2_s, CV, &med[3]);
            //asm volatile ("mul.hi.u32 %0, %1, " q2_s ";": "=r"(med[3]) : "r"(*d));
            CV = fn_MadLo(d[0], (cl_uint)q1_s, 0, &med[1]);
            ...
            CV = 0;
            //================================================================//
            CV = fn_Sub(r[0], med[0], CV, &r[0]);
            //asm volatile ("sub.cc.u32 %0, %0, %1;": "+r"(r[0]): "r"(med[0]));

#pragma unroll
            for (int i = 1; i < 5; ++i)
            {
                CV = fn_Sub(r[i], med[i], CV, &r[i]);
...
#pragma unroll
            for (int i = 5; i < 7; ++i)
            {
                CV = fn_Sub(r[i], 0, CV, &r[i]);
                //asm volatile ("subc.cc.u32 %0, %0, 0;": "+r"(r[i]));
            }
            CV = fn_Sub(r[7], 0, CV, &r[7]);
...
#pragma unroll
            for (int i = 5; i < 8; ++i)
            {
                CV = fn_Sub(r[i], (cl_uint)qhi_s, CV, &r[i]);
                //asm volatile ("subc.cc.u32 %0, %0, " qhi_s ";": "+r"(r[i]));
            }

            carry[0] = 0;
            CV = fn_Sub(0, 0, CV, &carry[0]);

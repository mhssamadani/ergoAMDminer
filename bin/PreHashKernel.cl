...
#include "C:\eeee\one\secp256k1/include/OCLdecs.h"////problem with relative path
...
#pragma unroll
        ...
        while(!j)
        {
            ...
            #pragma unroll
                ...
                {
                    ctx->b[ctx->c++]
                        = ((const uint8_t global *)(hashes + tid * NUM_SIZE_32))[j];
                }
            }
            ...
            for (int i = 0; i < NUM_SIZE_8; ++i)
            {
                    ((uint8_t global*)hashes)[(tid + 1) * NUM_SIZE_8 - i - 1]
                        = ((uint8_t *)ldata)[i];
            }
